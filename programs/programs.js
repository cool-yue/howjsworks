 ...
 */
 ...
 // 它被编译成一个函数然后调用
 // 它会导致函数对象被创建同时也有可能会执行

 // 那个就是程序的终止.但是在JavaScript中，对于源单位注册来接收事件或者信息或者导出一个函数,这个函数被
// 其他的源单位会用到,这些情况在JavaScript非常典型
// JavaScript也不强制要求程序应该以这种方式来写
// 但是这是一种伟大写程序的方式,JavaScript支持这种方式好过大多数语言

// 历史性记录:行内事件控制器工作稍微有点不同.源单位还是被当成一个函数体来对待
// 但是时间处理器函数这个创建的不会立即调用
