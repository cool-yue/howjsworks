 ...
 */
  ...
  */
   ...
   */
   ...
   // 一个不定期的JSLint的检查都是能够通过的
   // JSLint在正则中找到了一个bug而测试没有发现
   // 测试包含错误
   // 虚假的否定通常能够快速得到修复
   // 虚假的肯定是名垂千古地存在而不会解决
   // 这些测试给了我们假的信息,不是质量,什么能够测试你的测试呢？
   ...
   // 它让我对于写一个测试有一个好的感觉
   // 但是思考一下一个在add函数中的单一的加法被发现了一些细微的错误的可能性是现实的么？
   ...
   // signature是一个generator函数的数组,它是为predicate函数创造参数的
   ...
   // on_lost:一个针对失败地传一个verdict的测试的回调
   ...
   // 我设计了一个测试,这个测试可以包含多个一共工作的函数
   // 例如,demorgan测试用了random,mask,xor,or和eq
   // JSCheck传召一个随机整数,这个整数被用来创建随机的big integers,这个应用了DeMorgan Law
   ...
   // 我创建一个像这些大的集合的test
   // 这种testing的风格给了我更多的信息相比我从3+4得到的。
