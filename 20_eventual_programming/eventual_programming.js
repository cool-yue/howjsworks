 ...
 */
// 线程是最老的并行机制之一,现在仍然在用
// 多个线程是(真实或者虚拟)多个cpu在同一个时间运行,同时共享同一片内存
// 运行纯粹的函数,线程能够很好地工作
// 如果函数不纯粹,臭皮匠就来了!
...
// 在事件编程中的另外一种想法就是处理循环(processing loop),又叫Event loop(事件循环)或者消息循环(message loop)
...
// 它是一个非常可靠的模型
// 这里面有相关的macro queue,micro queue(浏览器环境),在node环境,浏览器环境等等
...
// 以上的轮次法则应用于处理循环所调用的回调函数,应用于它们调用的每个函数,直接或者间接的
...
// 所以任何违反这个轮次法则的函数必须被修正或者在单独的进程中被隔离
// 一个进程就像一个不分享它的内存的线程
// 所以一个回调函数发送一些工作给进程非常合理
// 并且那个进程当它完成的时候发送一条消息
// 信息被放进队列,事件驱动来传输

// JavaScript完全不知道当前系统多个进程的存在
// 进程是可以为当前系统中的javascript提供服务
// 进程是事件驱动模型中的重要的部分
// 所以下一个语言进程将会是第一类特性（就像函数对于javascript一样）
...
// 一个服务器的典型的工作流要复杂得多.
// 一个消息接收方或者处理器被调用,这个处理器可能又要跟其他的系统做交互
// 可能还需要定位到其他的机器
// 最后传回响应
// 有时候从一个系统获的信息后又要跟另外一个系统交互
...
// 第二个错误叫Promises.从最初形式上来看,promise的设计是聪明的。
// 它们被开发出来支撑安全,分布式(distributed)程序的开发
...
// 分离它们更好
// 当你发现你需要成片地粘贴代码的时候,那么这个就到了重构的时候了。

/**
 * requestors
 */

 // 我们应该模块化地设计.每个工作的单元的,可能是创建跟某一些服务器的请求或者数据库的请求或者一个进程的请求,这些应该是分开的函数
 // 一个函数只做一件事就有很强的内聚性
 ...
 // 我们使用单词requestor来描述这个函数
 // 这个函数接受一个回调和做一件事,这件事可能直到未来的一个turn的时候才完成。
